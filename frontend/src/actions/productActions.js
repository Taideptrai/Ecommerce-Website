...
export const getProductTest = () => async (dispatch) => {
  dispatch({
    type: GET_PRODUCTS_TEST_REQUEST,
  });
  try {
    const data = await Axios.get(`/api/products/test`);
    dispatch({ type: GET_PRODUCTS_TEST_SUCCESS, payload: data });
  } catch (error) {
    const message =
    error.response && error.response.data.message
      ? error.response.data.message
      : error.message;
  dispatch({ type: GET_PRODUCTS_TEST_FAIL, payload: message });
  }
};